...
export default function TimeslotTable() {
  ...
  if (isLoading) return <p className="text-center text-gray-500">🔄 Laddar tider...</p>;
  ...
  return (
    <div className="w-full mt-6">
      ...
      <div className="flex items-center justify-between mb-4 px-4">
        <button onClick={handlePrevWeek} className="text-2xl">←</button>
        <h2 className="text-lg font-semibold">
          {format(currentStartDate, 'd MMM')} - {format(currentEndDate, 'd MMM')}
        </h2>
        <button onClick={handleNextWeek} className="text-2xl">→</button>
      </div>
            ...
            <div key={key} className="border-r last:border-r-0 border-[#BDBDBD]">
              ...
              <div className="p-2">
                {slots.map((slot) => (
                  <div
                    key={slot.id}
                    className="text-sm border border-emerald-600 rounded-xl px-2 py-1 mb-2 text-center"
                  >
